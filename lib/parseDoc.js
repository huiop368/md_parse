...
var convertInlineCode = R.map(function (node) {
...
var isIntro = R.complement(R.either(function (node) {
  return node[0] === 'hr';
}, function (node) {
  return node[1] === 'API';
}));
var isDescription = R.complement(function (node) {
  return node[1] === 'API';
});
var parseIntro = R.takeWhile(isIntro);
var parseDescription = R.pipe(R.dropWhile(isIntro), R.takeWhile(isDescription), R.when(function (nodes) {
  return (nodes[0] || [])[0];
}, R.tail));
...
function parseDoc(fileName, content) {
  ...
  var enhancedDescription = convertInlineCode(description.length > 0 ? description : intro);

  var api = parseAPI(fileContentTree);
  var enhancedAPI = convertInlineCode(api.length > 0 ? api : []);
